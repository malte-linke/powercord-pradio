...
module.exports = class StreamPlayer {
  ...
  constructor(plugin) {
    ...
    this.usettings = setInterval(() => {
      ...
      let modal = plugin.settings.get("modal", false);
      if (!wasCoverEnabled && modal) { this.cover.inject(); wasCoverEnabled = true; } 
      else if (wasCoverEnabled && !modal) { this.cover.uninject(); wasCoverEnabled = false; }
  ...
  createMetaParser(url) {
    ...
    this.parser.on('metadata', (metadata) => {
      ...
      let [ artist, title ] = metadata.get("StreamTitle").split(' - ');
