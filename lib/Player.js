...
module.exports = class StreamPlayer {
  ...
  constructor(plugin) {
    ...
    let volume = plugin.settings.get('stream_volume', 10);
    this.setVolume(volume / 1.5);
    ...
    this.usettings = setInterval(() => {
      ...
      let volume = plugin.settings.get('stream_volume', 5);
      if (this.audio.volume !== volume && !muted) this.setVolume(volume / 1.5);
  ...
  createMetaParser(url) {
    ...
    this.parser.on('metadata', (metadata) => {
      ...
      console.log(`%c[PRadio]`, `color: cyan;`, `Now playing: ${streamTitle}`);
