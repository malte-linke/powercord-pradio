module.exports = class Cover {
  ...
  constructor(plugin) {
    ...
    muteButton.classList.add("button-14-BFJ", "enabled-2cQ-u7","button-38aScr","lookBlank-3eh9lL","colorBrand-3pXr91","grow-q77ONN");
    ...
    muteButton.appendChild(muteIconContainer);
    ...
    muteButton.addEventListener('click', () => {
      ...
      plugin.settings.set('muted', this.muted);
    });
  ...
  inject() {
    ...
    let injectionValid = ()  => {
      let valid = false;
      document.querySelector(`section.panels-j1Uci_`).childNodes.forEach(n => n == this.container ? valid = true : null);
      return valid;
    };

    let tryCounter = 0;
    let injectInterval = setInterval(() => {
      // if to many tries, stop
      if (tryCounter > 10) clearInterval(injectInterval);
  ...
  uninject() {
    try { this.container.parentNode.removeChild(this.container); } catch (e) { console.log(`%c[PRadio/Warn]`, `color:orange;`, `Failed to remove child! This error might be okay.`); }
